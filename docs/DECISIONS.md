...
**Estado:** Vigente y activación cerrada. `PRODUCTION_CANARY_DISPATCH_WAKE_SECRET` en Vercel Production+Preview; ruta `/api/dispatch-wake` validada E2E (doble wake → idle). Hobby limita el cron a `5 12 * * *`. Operativo: `mode=modal`, `kill_switch=false`, hard stop USD 20, `max_containers=1`, retries automáticos deshabilitados. Deuda: wake near-realtime (Pro o Webhook/`pg_net`), naming histórico `dispatch-modal-staging`/`production-canary`.
